...
import { ArrowRight, Lock, Mail, Loader2, AlertCircle, X } from 'lucide-react';
...
export default function Login({ onClose, onSwitchToSignup }: LoginProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(`AUTH_ERROR [${err.code}]: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[#020617]/90 backdrop-blur-xl">
      ...
      >
        ...
        <div className="flex flex-col items-center mb-10">
          ...
          <h1 className="text-3xl font-black tracking-tight text-white uppercase">Neural Access</h1>
          <p className="text-gray-500 text-xs font-bold uppercase tracking-[0.2em] mt-2">Synchronize Credentials</p>
        </div>
        ...
        <div className="mt-10 pt-10 border-t border-white/5 text-center">
          <p className="text-[10px] text-gray-600 font-black uppercase tracking-[0.2em]">
            New Operative? <button onClick={onSwitchToSignup} className="text-[#f3d36b] hover:underline ml-2 cursor-target">Request Identity</button>
          </p>
        </div>
